...
func TestGrokSubscriptionChannelRegistration(t *testing.T) {
	...
	if ChannelTypeDummy != 115 {
		t.Fatalf("ChannelTypeDummy = %d, want 115 (shifted after Apodex took over 114)", ChannelTypeDummy)
	}
...
func TestGrokSubscriptionAPIType(t *testing.T) {
	...
	if APITypeDummy != 40 {
		t.Fatalf("APITypeDummy = %d, want 40 (shifted after Apodex)", APITypeDummy)
	}

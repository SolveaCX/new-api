...
- 新板块用 **UTC+0 自然日聚合 + 落表 + 离线补缺** 的设计，读接口不扫历史。
...
| 转化率（当天口径，人） | `注册→激活率(当天)` = 该日注册者中当天首建 Key 人数 / 当日注册人数；`注册→首付率(当天)` = 该日注册者中当天首付人数 / 当日注册人数。恒 ≤100% |
...
2. **今日**：和历史日一样，已有快照不自动重算；后台任务只补缺失日期。后台任务和显式补数共用
...
- **每天 00:00 UTC**：只补最近 7 天内缺失日期，不刷新任何已有快照；

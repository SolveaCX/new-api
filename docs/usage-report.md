...
| 指标 | 口径 |
| --- | --- |
| 注册 | 当日 UTC+0 新增，`users.status = enabled(1)` **且** `email_verified_at > 0` |
| 激活(建 Key) | 用户**首次**创建 API Key（`tokens.created_time` 最早的一次）落在当日 |
| 首次付费 | 用户**首次**成功 top-up（`top_ups.status='success'` 且金额>0）落在当日 |

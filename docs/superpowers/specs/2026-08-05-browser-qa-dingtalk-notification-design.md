...
4. The module validates every status/count/URL against a closed contract, renders a Markdown report, and POSTs it to DingTalk. It retries transient network/HTTP/server failures with bounded backoff and requires a JSON response whose `errcode` is exactly zero.
...
The repository secret is named `STAGING_BROWSER_QA_DINGTALK_WEBHOOK`. The reusable workflow declares that secret, and `gcp-deploy-staging.yml` passes it explicitly. The manual `workflow_dispatch` path reads the same repository secret. The webhook is injected only into the notification step's environment; it is never placed in command-line arguments, job outputs, summaries, manifests, committed files, or messages.

For bootstrap, the existing GCP Secret Manager value `week-one-qa-dingtalk-webhook` may be piped directly into `gh secret set` without displaying it.
...
It never contains a Gmail address or alias, password, verification code, API key, cookie, authorization header, identity seed, or the DingTalk webhook.
...
- Missing webhook: fail closed with a generic error that does not reveal secret material.
...
- the Python sender renders only the fixed safe fields, retries transient failures, rejects nonzero `errcode`, and never includes the webhook in its message or exception text.

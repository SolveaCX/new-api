...
核算 universe 是“最终结算成功且消费日志已成功持久化”的同步请求，不是所有已向客户完成结算的请求，也不是对日志写失败请求的推算集合。生成 `captured` 财务快照必须同时满足：
...
### 5.1 Canonical 控制面与发布契约

- `options` 中恰有两个当前控制面保留键：`supplier_accounting_v1.activation` 和 `supplier_accounting_v1.mutations_enabled`。两者都是 strict schema-version-1 JSON，只接受各自固定字段和值域；未知字段、缺少必填字段、malformed JSON、不支持的 schema 或非法版本/值一律 fail closed，generic option API 不得写入。任一键不存在时只在读取时返回 synthetic state version 0，不插入行：activation 为没有 cutover 的 `disabled`，mutation gate 为 `enabled=false`。
- coverage gap 是可重叠、可跨日的具名 epoch。`active -> degraded` 必须在 activation CAS、命令完成的同一主库事务中幂等打开具名 gap；resolve 必须以 record CAS 精确关闭一个具名 gap、写入 finance disposition，并执行一次递增 activation state version 的 `degraded -> degraded`。其他 gap 保持不变；关闭最后一个 gap 仍保持 `degraded`，只有所有 gap 都已关闭且 disposition 非 pending 后，才可另行执行显式 `degraded -> active` CAS。
- 现有小表 `supplier_usage_daily_batch_runs` 是唯一 batch lifecycle 表，并在同一行内实现 candidate/published 双缓冲。candidate 侧保存可变的 status、lease/fence、cursor、候选 counters/warnings/error/request anchor；published 侧保存 `published_fence_token`、`published_at`、`published_persisted_log_snapshot_completeness` 和 `published_evidence_v1`。`published_evidence_v1` 必须是 strict schema-version-1 UTF-8 JSON，硬上限 `<=16 KiB`；未知/缺失字段、非法 enum、负数 counter、超限或与顶层 completeness 不一致都拒绝发布或读取。
- published report 恰有三个事实来源：金额按 `published_fence_token` 选择已发布 generation；completeness、全部 counters 和 warnings 只解码自 `published_evidence_v1`；coverage gaps 每次按已发布 `Asia/Shanghai` 日期/窗口重新查询 `supplier_accounting_coverage_gaps` 的 overlap。candidate status/counters/warnings/error/cursor 永远不是报表来源，candidate 失败、过期、fence 丢失或清理也不得改写既有 published snapshot。
- 支持的数据库契约是 SQLite、MySQL `>=5.7.8` 和 PostgreSQL `>=9.6`，同时覆盖 `model.LOG_DB = model.DB` 的共享库模式和配置 `LOG_SQL_DSN` 的日志分库模式。现有本地容量数据来自 MySQL 8.0.46/PostgreSQL 15.18，不能替代 MySQL 5.7.8/PostgreSQL 9.6 的最老支持版本 runtime/capacity 证明；该证明缺失期间 release/activation 继续阻断，不得表述为已经运行或通过。
...
- 每次 catch-up 最多处理一个最早从未发布的自然日，只处理 D-1 及以前；
...
- `newapi_supplier_accounting_never_published_days`：从 cutover 到前一日之间缺少 batch-run 行或已有行 `published_fence_token = 0` 的自然日数量，>1 表示当前至少积压两天；已有行必须构成从 cutover 开始的连续前缀，内部日期断层会令 observer fail closed，而不是低估积压；
...
G006 observer gate 从 `deploy/gcp/envs/prod/terraform.tfvars` 读取并契约校验当前 `router_max_instances=30`，再从该值推导 2 倍并发与五个 gauge 的 series 数。两库 DSN 必须解析为 loopback，且 AutoMigrate/清理前都要存在外部预置、与共享 token 匹配的 sentinel。成功运行生成 schema-v2 `supplier-observer.json`，记录时间、HEAD/dirty、Terraform 来源及 hash、真实测量值、unavailable fields 和 release blockers；workflow 只上传本次生成物，不复制 checked-in draft。`docs/evidence/supplier-g006-local-capacity.json` 及其中历史数值只是 provisional local draft，不能作为最终发布证据。

T+1 gate 只有 `RUN_SUPPLIER_T1_CAPACITY_TEST=1` 才运行；普通测试可因未设置该 flag 而 skip，但 flag 一旦开启，缺失/非法 `V`、任一 DSN、外部 sentinel token 或来源证据都必须失败。来源证据包括有序 RFC3339 measurement window、query/report reference 与 64 位 lowercase SHA-256，并写入 artifact；相对于 gate 校验时刻，measurement window end 必须满足 `now - 7×24h <= end <= now + 5m`。目标行数在 `max(1,000,000, 2V)` 后向上取整到 20 的倍数，保证下界与严格 95% internal / 5% business 同时成立。

manual workflow 是 activation 前强制 release gate 而非 merge CI，固定 `ubuntu-24.04` 和 digest-pinned MySQL/PostgreSQL，artifact 保留 90 天。`always()` 上传的失败或部分 artifact 仅用于诊断；只有完整 artifact 包含并通过 success-only `SUCCESS_MANIFEST.json` attestation 才满足 local gate。该 attestation 必须绑定 current `GITHUB_SHA` 和 clean working tree，精确验证 `supplier-observer.json`、`supplier-t1-mysql.json`、`supplier-t1-postgres.json` 三份 JSON 的 schema/evidence class/dialect、workflow-window timestamps 和 SHA-256 digests，并记录完整 runner metadata。

`SUCCESS_MANIFEST.json` 只证明本次 local capacity run 的完整性与成功执行。workflow 会把 operator 提供的 source reference、claimed source SHA-256、measurement window 和 `V` 绑定进生成 JSON，但没有 credential 或 storage trust path 可独立认证该外部生产来源。release approver 必须另行取得 immutable source、重新计算并匹配 SHA-256，再把 reference/window/`V` 与两份 T+1 JSON 对账；该外部验证被记录前 activation 继续显式阻断。即使完成该验证，production-distribution EXPLAIN/rows-read、Cloud SQL CPU/locks/lag 与 approved staging/live fire/resolve 仍阻断 activation。

按 golden payload 的 business 330B、internal 320B 和 95% internal / 5% business，保守 all-captured 平均为 320.5B/成功日志；若生产 p99 日成功日志量为 `V`，原始 payload 增量为 `320.5 * V` B/日、`9,615 * V` B/30 天，尚未包含数据库页、行头、备份等开销。由于本任务禁止生产访问，真实 p99 `V`、生产 CPU/连接/锁、MySQL 5.7.8/PostgreSQL 9.6 和 Cloud Monitoring fire/resolve/通知证据继续作为发布阻断。

## 8. 历史数据边界

canonical 控制面是 `options` 中版本化的 activation document。缺少 activation 时系统保持 disabled，既不生成 cutover timestamp，也不因启动或 `SUPPLIER_ACCOUNTING_CUTOVER_AT` 自动写入状态；该环境变量只用于 readiness assertion。
...
只可经 Root 的显式、审计化 `adopt_legacy` 命令一次性采用；adoption 精确保留历史 cutover 并创建 active activation document。没有遗留键时不得合成、猜测或回填它。新部署走 prepare → arm（显式未来 cutover）→ activate，不依赖 legacy key。

- 切点前日志不扫描、不使用今天的定价猜算历史利润；
- 切点后没有 `supplier_accounting_v1` 快照的消费日志会被扫描但跳过；
- scheduler catch-up 只选择最早 `published_fence_token = 0` 的从未发布日期，不重跑已发布日期，也不接受指定日期；只有“已发布且 `persisted_log_snapshot_completeness=incomplete`”可由 Root 通过日报受控入口，以 mutation gate、fresh secure verification、expected published fence 和稳定幂等键发起 rerun；
- 报表必须展示 coverage start 和 freshness，避免把部分覆盖误认为完整历史。

上线时由受审计控制面 arm 唯一的未来 cutover，且时间点晚于全部兼容 Router 切流完成；所有 Go 服务上的可选 assertion 必须与 persisted activation 一致。缺 activation 时只读取 synthetic disabled 且不得激活；malformed activation 或 assertion 不一致都 fail closed，不允许“最先启动者写入”的竞态语义。
...
- `persisted_log_snapshot_completeness` 只描述已持久化 universe 内的行级快照完整性；`supplier_accounting_coverage_gaps` 描述已知时间区间缺口，两者是独立轴。`finance_attention_required` 是读取时由“completeness incomplete 或存在相交 unresolved coverage gap”派生的非持久化信号，不得存成第三套事实或用一个轴覆盖另一个轴；
...
2. 验证 Router 与 Console 使用同一主库、日志库配置一致；由受审计控制面 arm 唯一未来 cutover，并让所有 Go 服务的可选 readiness assertion 与之相同；

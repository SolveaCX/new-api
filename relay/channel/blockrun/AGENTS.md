...
- **OpenAI Responses** → `/v1/responses`，请求与 JSON/SSE 响应保持 Responses 原生语义，`DoResponse` 委托内嵌的 `openai.Adaptor.DoResponse`。上游原生 `response.completed.usage` 用于结算；BlockRun 会拒绝 `stream_options`，因此在 Responses 请求转换阶段和参数覆盖后的最终发送边界都移除该字段。

...
func TestBlockRunDoResponse_TruncatedToolOnlyKnownLimitation(t *testing.T) {
	ensureResponseStreamTimeout(t)
	upstream := "event: response.output_item.done\n" +
		`data: {"type":"response.output_item.done","output_index":0,"item":{"id":"ct_truncated","type":"custom_tool_call","name":"shell","input":"pwd"}}` + "\n\n"
	rec := httptest.NewRecorder()
	c, _ := newResponseTestContext(rec, "/v1/responses")
	info := responsesInfo(true)
	info.SetEstimatePromptTokens(6)
	...
	if usage.PromptTokens != 0 || usage.CompletionTokens != 0 || usage.TotalTokens != 0 {
		t.Fatalf("tool-only truncation limitation changed unexpectedly: %#v", usage)
	}
	for _, want := range []string{"event: response.output_item.done", `"id":"ct_truncated"`, `"type":"custom_tool_call"`, `"name":"shell"`, `"input":"pwd"`} {
		if !strings.Contains(rec.Body.String(), want) {

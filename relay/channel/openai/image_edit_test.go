...
// re-serializes multipart image edit requests with the file and standard
// fields intact, both when the form was already parsed and when it must be
// re-parsed from the reusable body. On this fork the passthrough conversion
// deliberately strips the image streaming fields (stream / partial_images)
// because new-api only synthesizes SSE for opt-in channels (e.g. blockrun);
// forwarding them would 400 upstreams that don't support image streaming.
func TestConvertImageEditRequestMultipart(t *testing.T) {
	...
	convertAndReplay := func(t *testing.T, c *gin.Context, prompt string) {
		...
		// Image streaming fields are stripped on the passthrough conversion path
		// (blockrun-era behavior): upstreams that don't synthesize streaming
		// would 400 otherwise. new-api opt-in channels synthesize SSE instead.
		require.Empty(t, replayedRequest.PostForm.Get("stream"))
		require.Empty(t, replayedRequest.PostForm.Get("partial_images"))

...
func (s *SubscriptionFunding) PoolType() string {
	if s.poolType == model.SubscriptionPoolMedia {
		return model.SubscriptionPoolMedia
	}
	return model.SubscriptionPoolQuota
}

func isSubscriptionMediaModel(modelName string) bool {
...
func mediaCreditsForQuota(quota int64) int64 {
	if quota <= 0 {
		return 0
	}
...
func (s *SubscriptionFunding) PreConsume(_ int) error {
	if s.PoolType() == model.SubscriptionPoolMedia {
		credits := mediaCreditsForQuota(s.amount)
		res, err := model.PreConsumeUserSubscriptionMedia(s.requestId, s.userId, s.modelName, credits)
		if err != nil {
			return err
		}
...
func (s *SubscriptionFunding) Settle(delta int) error {
	...
	if s.PoolType() == model.SubscriptionPoolMedia {
		finalCredits := mediaCreditsForQuota(s.amount + s.extraQuota + int64(delta))
		currentCredits := s.preConsumed + s.extraWeighted
		if err := model.SetUserSubscriptionMediaPreConsume(s.requestId, finalCredits); err != nil {
			return err
		}
		s.settlementDelta += finalCredits - currentCredits
		s.extraWeighted = finalCredits - s.preConsumed
...
func (s *SubscriptionFunding) ReserveExtra(delta int64) error {
	...
	if s.PoolType() == model.SubscriptionPoolMedia {
		finalCredits := mediaCreditsForQuota(s.amount + s.extraQuota + delta)
		currentCredits := s.preConsumed + s.extraWeighted
		if err := model.SetUserSubscriptionMediaPreConsume(s.requestId, finalCredits); err != nil {
			return err
		}
		s.extraQuota += delta
		s.extraWeighted += finalCredits - currentCredits
		if s.extraWeighted < 0 {
			s.extraWeighted = 0
		}
		return nil
...
func (s *SubscriptionFunding) SettlementDelta() int64 {
	if s.PoolType() == model.SubscriptionPoolMedia {
		return s.settlementDelta
	}
	return 0
}

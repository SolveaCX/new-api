...
export function getModelPromotionLabel(promotion: ModelPromotion, t: TFunction): string {
...
export function sortModelsByPromotion(models: readonly ModelAccessModel[]): ModelAccessModel[] {
  return models.map((model, index) => ({ model, index, priority: modelPromotionPriority(model.id) })).sort((a, b) => a.priority - b.priority || a.index - b.index).map(({ model }) => model)
}

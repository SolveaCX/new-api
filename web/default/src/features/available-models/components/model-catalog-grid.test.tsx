...
const { ModelCatalogGrid, getEffectiveVisibleCatalogCount, getNextVisibleCatalogCount, MODEL_CATALOG_PAGE_SIZE } =
...
describe('ModelCatalogGrid', () => {
  ...
  test('renders only the first page and offers incremental expansion', () => {
    const models = Array.from(
      { length: MODEL_CATALOG_PAGE_SIZE + 1 },
      (_, i) => buildModel({ id: `model-${i + 1}` })
    )
  ...
  test('advances paging by one page, capped at the total', () => {
    ...
    expect(
      getNextVisibleCatalogCount(MODEL_CATALOG_PAGE_SIZE, 999)
    ).toBe(MODEL_CATALOG_PAGE_SIZE * 2)
  })
})

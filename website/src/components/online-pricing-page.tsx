...
import { STANDARD_SUBSCRIPTION_LIMITS } from "@/lib/subscription-pricing";
...
const paymentMethods = [
  { kind: "card" },
  {
    height: 450,
    kind: "pix",
    src: "/assets/logos/payment/pix.jpg",
    width: 800,
  },
  {
    height: 900,
    kind: "upi",
    src: "/assets/logos/payment/upi.jpg",
    width: 1200,
  },
  {
    height: 77,
    kind: "alipay",
    src: "/assets/logos/payment/alipay.svg",
    width: 298,
  },
  {
    height: 96,
    kind: "usdc",
    src: "/assets/logos/payment/usdc.svg",
    width: 96,
  },
] as const;
...
const plans = [
  {
    ...
    prices: {
      BRL: 49.9,
      JPY: 1_500,
      USD: STANDARD_SUBSCRIPTION_LIMITS.go.priceUsd,
    },
  },
  {
    ...
    prices: {
      BRL: 149.9,
      JPY: 4_500,
      USD: STANDARD_SUBSCRIPTION_LIMITS.pro.priceUsd,
    },
  },
  {
    ...
    prices: {
      BRL: 499,
      JPY: 15_000,
      USD: STANDARD_SUBSCRIPTION_LIMITS.max.priceUsd,
    },
  },
] as const;
...
function formatPaymentPrice(
  locale: Locale,
  prices: Readonly<Record<PaymentCurrency, number>>,
): string {
...
function subscriptionSignupHref(plan: PlanName, paymentMethod?: string) {
  ...
  return consoleUrl(
    "/sign-up",
    `redirect=${encodeURIComponent(`/wallet?${redirectParams}`)}`,
  );
}
      ...
      <div className="wrap">
        ...
        <div className="tiers">
          {displayedPlans.map((plan) => {
            ...
            return (
              <div
                className={`tier${plan.hot ? " hot" : ""}${plan.limitedOffer ? " limited-offer" : ""}`}
                key={plan.name}
              >
                {plan.hot && (
                  <div className="badge">{copy.pricing.mostPopular}</div>
                )}
                {plan.limitedOffer && (
                  <div className="badge limited">
                    {copy.pricing.limitedOffer}
                  </div>
                )}
                ...
                <a
                  className={`btn ${plan.hot ? "primary" : "white"} tcta`}
                  href={plan.href}
                >
                  {planCopy.cta}
                </a>
                <div className="tval">
                  <span className="tglabel">
                    {copy.pricing.textModelsLabel}
                  </span>
                  <div className="tgsub twindow">{planCopy.monthlyQuota}</div>
                  <div className="tdiv" />
          ...
          <div className="tier">
            ...
            <div className="tprice">
              <b className="tcustom">{copy.pricing.customPrice}</b>
            </div>
            <Link
              className="btn black tcta"
              href={localizePath("/contact", props.locale)}
            >
              {copy.pricing.enterpriseCta}
            </Link>
            <div className="tval">
              <p style={{ margin: 0 }}>{copy.pricing.enterpriseBody}</p>
            </div>
          </div>
        </div>
        <OnlinePaymentMethodPicker
          ...
          methods={paymentMethods.map((method, index) => ({
            ...method,
            ctaHref: subscriptionSignupHref(
              "pro",
              subscriptionPaymentMethod(method.kind),
            ),

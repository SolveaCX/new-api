...
function FlatkeyModelDetailPage(props: {
  ...
  // suffixes while still preserving live catalog values verbatim.
  const localizeHeroPrice = (value: string) => value
    .replaceAll("/ image", props.t("/ image"))
    .replaceAll("/ second", props.t("/ second"))
...
function buildFlatkeyPriceRows(
  ...
  if (config.generator) {
    ...
    if (perUnit) {
      const official = perUnit.configured ?? perUnit.value;
      ...
      const prefix = perUnit.from ? "from " : "";
      return {
        note,
        rows: [{ label: t(labelKey), flatkey: `${prefix}${formatUsdPrice(flatkey)} ${t(unitKey)}`, official: `${prefix}${formatUsdPrice(official)} ${t(unitKey)}`, flatkeyPercent: pricePercent(flatkey, official), officialPercent: 100 }],
      };
    }

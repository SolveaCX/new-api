    ...
    <>
      <label className="block text-sm font-semibold text-[#2c2d33] dark:text-white/88">
        ...
        <textarea
          ...
          className="mt-2 min-h-[140px] w-full resize-y rounded-xl border border-violet-500/16 bg-white/70 p-4 font-mono text-sm leading-6 font-medium text-[#20222a] outline-none transition focus:border-violet-500/35 focus:bg-white focus:ring-4 focus:ring-violet-500/10 dark:bg-white/[0.04] dark:text-white/86"
        />
      </label>
        ...
        <div className="flex flex-wrap gap-2.5">
          {quickPrompts.map((item) => (
            <button
              ...
              className="rounded-lg border border-violet-500/16 bg-white/70 px-3.5 py-2 text-[13px] font-semibold text-[#4f4d56] transition hover:border-violet-500/35 hover:bg-violet-500/8 hover:text-violet-700 dark:bg-white/[0.04] dark:text-white/74 dark:hover:text-white"
            >
              {props.t(item)}
            </button>
          ))}
      ...
      <div className="mt-6">
        <div className="rounded-xl border border-violet-500/14 bg-violet-500/[0.035] p-4 sm:p-5 dark:bg-white/[0.025]">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div className="text-sm font-semibold text-[#2c2d33] dark:text-white/88">{props.t("Advanced Options")}</div>
          </div>
...
function GeneratorFieldControl(props: {
  ...
  if (props.field.type === "boolean") {
    return (
      <label className="flex min-h-[4.55rem] min-w-0 cursor-pointer items-center justify-between gap-4 rounded-xl border border-violet-500/14 bg-white/78 px-4 py-3 text-sm font-semibold text-[#5d5b64] transition hover:border-violet-500/35 hover:bg-white dark:bg-white/[0.04] dark:text-white/74">
  ...
  if (props.field.type === "number" && props.field.name === "n") {
    ...
    return (
      <label className="grid min-w-0 gap-2 text-[11px] font-semibold tracking-normal text-muted-foreground uppercase">
        <span>{props.t(props.field.label)}</span>
        <span className="grid h-11 grid-cols-[2.75rem_1fr_2.75rem] overflow-hidden rounded-lg border border-violet-500/14 bg-white/78 text-base font-semibold tracking-normal text-[#20222a] transition focus-within:border-violet-500/35 focus-within:ring-4 focus-within:ring-violet-500/10">
          <button
            type="button"
            onClick={() => update(numericValue - 1)}
            className="grid place-items-center border-r border-violet-500/10 text-[#5d5b64] transition hover:bg-violet-500/8 hover:text-violet-700"
          >
          ...
          <button
            type="button"
            onClick={() => update(numericValue + 1)}
            className="grid place-items-center border-l border-violet-500/10 text-[#5d5b64] transition hover:bg-violet-500/8 hover:text-violet-700"
          >
  ...
  if (canUseSegmented) {
    return (
      <div className="grid min-w-0 gap-2 text-[11px] font-semibold tracking-normal text-muted-foreground uppercase">
        <span>{props.t(props.field.label)}</span>
        <div className={`${segmentedGridClass(props.field)} min-h-11 gap-1.5 rounded-lg border border-violet-500/14 bg-violet-500/[0.055] p-1`}>
            ...
            return (
              <button
                ...
                className={`min-h-9 min-w-0 rounded-[0.72rem] px-3 py-2 text-[13px] leading-5 font-extrabold tracking-normal transition ${
                  active
                    ? "bg-white text-violet-700 shadow-[0_8px_18px_-14px_rgba(76,29,149,.7)] ring-1 ring-violet-500/20"
                    : "text-[#5d5b64] hover:bg-white/75 hover:text-violet-700"
                }`}
              >
      ...
      {props.field.type === "select" ? (
        <span className="relative block">
          <select
            value={String(props.value)}
            onChange={(event) => props.onChange(coerceGeneratorValue(props.field, event.target.value))}
            className="h-11 w-full min-w-0 appearance-none rounded-lg border border-violet-500/14 bg-white/78 px-3.5 pr-9 text-sm font-semibold tracking-normal text-[#20222a] outline-none transition focus:border-violet-500/35 focus:ring-4 focus:ring-violet-500/10"
          >
            {(props.field.options ?? []).map((item) => (
        ...
        <input
          ...
          value={String(props.value)}
          onChange={(event) => props.onChange(coerceGeneratorValue(props.field, event.target.value))}
          className="h-11 w-full min-w-0 rounded-lg border border-violet-500/14 bg-white/78 px-3.5 text-sm font-semibold tracking-normal text-[#20222a] outline-none transition focus:border-violet-500/35 focus:ring-4 focus:ring-violet-500/10"
        />
      )}
      {props.field.help ? <span className="text-[10px] font-medium tracking-normal text-[#8b8891] normal-case">{props.t(props.field.help)}</span> : null}
    </label>
...
function OutputPreview(props: {
  ...
  return (
    <div className={props.kind === "video" ? "overflow-hidden rounded-2xl border border-violet-500/16 bg-[#10131a] p-2 text-white shadow-[0_18px_42px_-32px_rgba(15,15,18,.8)]" : "overflow-hidden rounded-2xl border border-violet-500/16 bg-white/78 p-2 text-[#0B0B0F] shadow-[0_18px_42px_-34px_rgba(76,29,149,.65)] dark:bg-white/[0.04]"}>
      <div className={`relative overflow-hidden rounded-xl ${props.kind === "video" ? "aspect-video bg-[#171b24]" : "aspect-[16/10] bg-[#11131a]"}`}>
      ...
      <div className={`grid gap-1 px-2 pt-3 pb-1 text-xs leading-5 ${props.kind === "video" ? "" : "text-[#3f3d46] dark:text-white/74"}`}>
        ...
        <span className={`line-clamp-2 ${props.kind === "video" ? "text-white/72" : "text-muted-foreground"}`}>{props.prompt}</span>
      </div>
    </div>
  );
}

 ...
 *
 * Only model-provenance video assets are declared: the industry template clips
 * are playable references, but they are not claimed as generated by the model
 * on the page. Image models' library entries are stills.
 */
function promptLibraryVideoSchema(
  ...
  // clips, not renders from this model. Do not publish VideoObject metadata for
  // them or let search engines mistake a template clip for model output.
...
function ModelShowcase(props: {
  ...
  // industry-matched reference images. Video pages use playable reference
  // clips. The workbench still has exactly one representative industry
  // template. Keeping these surfaces
  ...
  // reference clips. Legacy generated clips remain available only as a
    ...
    props.kind === "image"
      ...
      : props.kind === "video"
        ? videoTemplates.map((template, index) => ({
            ...
            // Video models should show a playable clip. The badge and copy
            // still make provenance clear: this is a reference/template clip,
            // not a claim that this model generated the asset.
            video: template.video,
            kind: "video",
            isPromptTemplate: true,
            ratio: template.ratio,
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-6 grid gap-5">
          {scenes.map((scene, index) => {
            ...
            const isVideo = sampleKind(scene) === "video" && Boolean(scene.video);
            return (
              <div
                key={scene.id}

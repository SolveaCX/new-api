...
describe("welcome promotion modal", () => {
  test("uses a limited-time free label for the English free-trial offer", () => {
    expect(WELCOME_PROMO_COPY.en.freeLimited).toBe("Free for a limited time");
  });
  ...
  test("provides translated copy for every website locale", () => {
    ...
    for (const locale of LOCALES) {
      ...
      expect(copy.freeLimited.length).toBeGreaterThan(1);

          ...
          <div className="relative z-10 grid min-h-0 flex-none grid-cols-1 grid-rows-none gap-4 sm:max-lg:grid-cols-2 [@media(max-width:1023px)_and_(orientation:landscape)_and_(min-width:360px)]:grid-cols-2 max-[479px]:gap-0 sm:gap-4 lg:grid lg:h-full lg:flex-none lg:grid-rows-none lg:grid-cols-[minmax(0,1fr)_377px] lg:items-center lg:gap-8">
            <div>
              ...
              <div className={["mt-4 flex w-full gap-2 sm:mt-5 sm:gap-3 sm:max-lg:mt-4 sm:max-lg:gap-2 lg:w-auto lg:flex-wrap max-[479px]:mt-3 max-[479px]:flex-col", desktopActionMarginClass].join(" ")}>
                <a href={consoleUrl("/dashboard")} onClick={navigateToHref} className="inline-flex h-[clamp(32px,6vh,44px)] min-w-0 flex-1 items-center justify-center rounded-full bg-black px-3 text-xs font-medium !text-white transition hover:bg-black/80 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black max-[479px]:h-[clamp(30px,6vh,44px)] sm:h-[clamp(32px,6vh,44px)] sm:px-5 sm:text-sm sm:max-lg:h-[clamp(32px,6vh,44px)] sm:max-lg:px-3 sm:max-lg:text-xs lg:h-12 lg:flex-none lg:px-7 lg:text-base max-[479px]:w-full max-[479px]:flex-none">{copy.getApiKey}<span aria-hidden="true" className="ml-2 text-lg leading-none !text-white lg:ml-3 lg:text-xl">↗</span></a>

...
import { getOnlineHomeToolText, getOnlineStaticCopy, getOnlineStaticText } from "@/lib/online-static-copy";
...
const videoCards = [
  ["v1.1", "Seedance 2.5", "Image to Video", "bytedance · 1080p · t2v + i2v", "Early access", "/playground?model=seedance-2.5", true],
  ["v1.2", "Seedance 2.0", "Image to Video", "bytedance · i2v", "Usage-based", "/playground?model=seedance-2.0-i2v", false],
  ["v1.3", "Veo 3.1", "Text to Video", "google · t2v + audio", "Coming soon", "/playground?model=veo-3.1", false],
] as const;
...
const modelWallRows = [
  modelTiles,
  [
    ["seedance-2.5", "bytedance · video · t2v + i2v", "Early access", true, false],
...
function PixelGrid(props: {
  ...
  for (let index = 0; index < props.n; index += 1) {
    ...
    const background = rnd() < 0.07 ? accent : colors[Math.floor(rnd() * colors.length)];
    pixels.push(
      <i
        key={`${key}-${index}`}
        style={{
...
function renderHeroTitle(locale: Locale, fallback: ReactNode) {
  ...
  const match = value.match(/^(.*?)<br><span class="price">(.*?)<span class="toolLine">(.*?)<\/span><span class="costLine">(.*?)<\/span><\/span>$/);
...
function renderToolsTitle(locale: Locale, fallback: ReactNode) {
  ...
  if (!value) return fallback;
  const match = value.match(/^<span class="tools-title-line">(.*?)<\/span><br>(.*)$/);
  if (!match) return renderLineBreakText(value.replace(/<[^>]+>/g, ""));
  return (
    <>
...
export function OnlineHomePage(props: { locale: Locale }) {
  const copy = getOnlineStaticCopy(props.locale);
  const zh = props.locale === "zh";
  const t = (key: string, fallback: string) => getOnlineStaticText(props.locale, key, fallback);
  const ht = (key: string, fallback: string) => getOnlineHomeToolText(props.locale, key, fallback);
  ...
  return (
    <OnlineStaticShell locale={props.locale} pathname="/">
      <style>{`
        ...
        .tools-intro{position:relative;min-height:100svh;overflow:hidden;border-bottom:1px solid var(--tool-line)}
      ...
      <header className="hero heroUnified">
        ...
        <div className="heroGrid">
          ...
          <div className="heroSavings" aria-label="Flatkey replaces separate AI subscriptions">
            <div className="saveRow"><s>{copy.home.savings[0]}</s><small>{ht("tools.hero.modelPrice", "$20–200 / seat")}</small></div>
            <div className="saveRow"><s>{copy.home.savings[1]}</s><small>{ht("tools.hero.toolPrice", "$49–500 / month")}</small></div>
            <div className="saveRow"><s>{copy.home.savings[2]}</s><small>{ht("tools.hero.automationPrice", "$20–250 / month")}</small></div>
            <div className="balanceCard"><span>{copy.home.balance}</span><strong>{copy.home.pay}</strong><p>{copy.home.invoice}</p></div>
          </div>
        </div>
      </header>
      <section className="tools-intro">
        <div className="tools-intro-inner">
          <div>
            ...
            <h2 className="tools-title">{renderToolsTitle(props.locale, copy.home.toolsTitle)}</h2>
            ...
            <div className="agent-label">
              ...
              <div className="agent-marks">
                ...
                <span className="agent-mark more">{ht("tools.intro.more", "+ more")}</span>
              </div>
            </div>
            <button className="agent-command" id="copy-flatkey-setup" type="button" data-action="copy-flatkey-setup" aria-label="Copy Flatkey setup prompt" data-copy="Set up Flatkey from https://flatkey.ai/SKILL.md">
              <span className="prompt" aria-hidden="true">&gt;</span>
              <code>{copy.home.toolsCommand}</code>
              <span className="copy-state" aria-hidden="true">
                <span className="copy-label">{ht("tools.intro.copyButton", "Copy")}</span>
                <span className="copy-icon-wrap">
                  <svg className="copy-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" /><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2" /></svg>
                  <svg className="check-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6 9 17l-5-5" /></svg>
                </span>
              </span>
            </button>
            <div className="agent-note">{ht("tools.intro.note", "No new seat. No separate provider keys. Every model and tool call lands on the same ledger.")}</div>
            <div className="agent-links">
              <a href={consoleUrl("/api-marketplace")}>{ht("tools.intro.explore", "Explore tools →")}</a>
              <a href="https://docs.flatkey.ai/" target="_blank" rel="noopener noreferrer">{ht("tools.intro.docs", "Read the docs")}</a>
            </div>
          </div>
          <div className="terminal">
            <div className="terminal-head"><i className="terminal-dot red" /><i className="terminal-dot yellow" /><i className="terminal-dot green" /><span>{copy.home.terminal.title}</span></div>
            <div className="terminal-body">
              <div className="term-brand">
                <div className="term-bot" />
                <div><b>flatkey tools</b><span>{ht("tools.terminal.tagline", "one key · models + tools")}</span></div>
              </div>
              <div className="term-command">{ht("tools.terminal.command", "❯ Find 14 competitors, research their launches, identify decision makers, waterfall-enrich contacts, and prepare a sourced GTM brief.")}</div>
              ...
              <div className="term-summary">
                <div className="term-stat"><small>{copy.home.terminal.successfulLabel}</small><b>{copy.home.terminal.successfulValue}</b></div>
                <div className="term-stat"><small>{copy.home.terminal.runtimeLabel}</small><b>{copy.home.terminal.runtimeValue}</b></div>
                <div className="term-stat"><small>{copy.home.terminal.invoiceLabel}</small><b>{copy.home.terminal.invoiceValue}</b></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="tool-universe">
        <div className="universe-inner">
          <div className="tools-kicker" style={{ justifyContent: "center" }}>{copy.home.universeKicker}</div>
          <h2 className="universe-title" style={{ marginTop: 20 }}>{renderLineBreakText(ht("tools.universe.title", "Every model. Every tool.<br>One key."))}</h2>
          <p className="universe-copy">{copy.home.universeCopy}</p>
          <div className="hub" aria-hidden="true">
            <div className="hub-core">
              <div className="hub-brand">
                <img className="hub-logo" src="/assets/flatkey-mark.svg" alt="" />
                <div className="hub-brand-copy"><strong>flatkey</strong><span>{ht("tools.universe.router", "Unified model + tool router")}</span></div>
                <span className="hub-live">{ht("tools.universe.live", "ROUTING LIVE")}</span>
              </div>
              <div className="hub-metrics">
                <span className="hub-metric"><b>300+</b><small>{ht("tools.universe.models", "official models")}</small></span>
                <span className="hub-metric"><b>1,000+</b><small>{ht("tools.universe.tools", "AI tools")}</small></span>
                <span className="hub-metric"><b>1</b><small>{ht("tools.universe.balance", "shared balance")}</small></span>
              </div>
            </div>
          </div>
          <div className="capability-map">
            {providerRows.map((row, rowIndex) => (
              <div className={`capability-row ${["models", "social", "crawl", "gtm"][rowIndex]}`} data-section={["models", "social", "crawl", "gtm"][rowIndex]} key={rowIndex}>
                <div className="provider-cards">
                  {row.map(([src, name, desc]) => (
                    <article className="provider-card" key={name}>
                      <i className="provider-icon"><img src={`/assets/${src}`} alt={name} /></i>
                      <div><b>{name}</b><span>{desc}</span></div>
                    </article>
                  ))}
                  {rowIndex === 2 && (
                    <article className="provider-card">
                      <i className="provider-icon voc-mark"><span>VOC</span></i>
                      <div><b>VOC AI</b><span>voice of customer</span></div>
                    </article>
                  )}
                  {rowIndex === 3 && (
                    <>
                      <Link className="provider-card more-card models-more" href={localizePath("/models", props.locale)} aria-label="Explore more than 300 official AI models">
                        <i className="provider-icon more-icon" aria-hidden="true"><span /></i>
                        <div><b>{renderEmStart(ht("tools.universe.moreModels", "<em>More</em> models"))}</b><span>{ht("tools.universe.moreModelsCount", "300+ official models")}</span></div>
                      </Link>
                      <a className="provider-card more-card tools-more" href={consoleUrl("/api-marketplace")} aria-label="Explore more than 1,000 AI tools">
                        <i className="provider-icon more-icon" aria-hidden="true"><span /></i>
                        <div><b>{renderEmStart(ht("tools.universe.moreTools", "<em>More</em> tools"))}</b><span>{ht("tools.universe.moreToolsCount", "1,000+ available")}</span></div>
                      </a>
                    </>
                  )}
                </div>
          ...
          <div className="tool-values">
            <article className="tool-value"><small>{ht("tools.value.authEye", "ONE AUTH")}</small><b>{ht("tools.value.authTitle", "One Flatkey key")}</b><p>{ht("tools.value.authCopy", "No provider credentials scattered across agents, repos, and environments.")}</p></article>
            <article className="tool-value"><small>{ht("tools.value.contractEye", "ONE CONTRACT")}</small><b>{ht("tools.value.contractTitle", "Normalized calls and results")}</b><p>{ht("tools.value.contractCopy", "Consistent schemas, error envelopes, request IDs, and usage records.")}</p></article>
            <article className="tool-value"><small>{ht("tools.value.policyEye", "ONE POLICY")}</small><b>{ht("tools.value.policyTitle", "Budgets and allowlists")}</b><p>{ht("tools.value.policyCopy", "Control which agents can call which tools and how much they may spend.")}</p></article>
            <article className="tool-value"><small>{ht("tools.value.billEye", "ONE BILL")}</small><b>{ht("tools.value.billTitle", "Pay for successful work")}</b><p>{ht("tools.value.billCopy", "Models and tools share one balance; failed Flatkey-side calls cost nothing.")}</p></article>
          </div>
        </div>
      </section>
      <section className="rel" id="reliability">
        <PixelGrid cell={18} cols={8} n={9} rows={3} seed={139} style={{ right: 38, top: 20 }} />
        <div className="relIn">
          <div>
            <div className="kick2">{t("rel.kick", "RELIABILITY · SLA, IN WRITING")}</div>
            <h2 className="display">{renderLineBreakText(t("rel.h2", "Stability you can<br>hold us to."))}</h2>
            <p className="sub" style={{ marginTop: 14, maxWidth: 440 }}>{t("rel.p", "One request, five upstream channel classes. When a channel degrades, the router reroutes automatically — and the SLA is a public document, not a sales promise.")}</p>
            <div className="relStats">
              <div><b className="num">99.5%</b><span>{renderLineBreakText(t("rel.s1", "signed SLA — public terms →").replace(/<a[^>]*>/g, "").replace(/<\/a>/g, ""))}</span></div>
              <div><b className="num">5</b><span>{t("rel.s2", "channel classes, automatic failover")}</span></div>
              <div><b className="num">$0</b><span>{t("rel.s3", "balance consumed by flatkey-side errors")}</span></div>
            </div>
            <div className="home-section-actions"><Link className="btn black big" href={localizePath("/status", props.locale)}>{t("rel.cta1", "Live status →")}</Link><Link className="btn white big" href={localizePath("/sla", props.locale)}>{t("rel.cta2", "SLA & service credits")}</Link></div>
          </div>
          <div className="relChart">
            <div className="rcHead"><span>{t("rel.ct", "UPTIME · SINGLE CHANNEL VS FLATKEY MESH · LIVE")}</span><span className="pill mint"><span className="dot" />LIVE</span></div>
            <svg viewBox="0 0 560 220" preserveAspectRatio="none">
              <line x1="0" y1="30" x2="560" y2="30" stroke="#0B0B0F14" /><line x1="0" y1="90" x2="560" y2="90" stroke="#0B0B0F14" /><line x1="0" y1="150" x2="560" y2="150" stroke="#0B0B0F14" />
              <path d="M0,34 L60,32 L90,36 L120,120 L150,140 L170,60 L220,38 L260,34 L300,110 L330,170 L360,150 L390,44 L440,36 L480,100 L510,60 L560,38" fill="none" stroke="#D97706" strokeWidth="2.2" strokeLinejoin="round" />
              <path d="M0,26 L80,24 L160,27 L240,24 L320,28 L400,24 L480,26 L560,24" fill="none" stroke="#15803D" strokeWidth="2.8" strokeLinejoin="round" />
              <text x="8" y="20" fontSize="11" fill="#15803D" fontWeight="700" fontFamily="monospace">flatkey mesh 99.98%</text>
              <text x="300" y="196" fontSize="11" fill="#D97706" fontWeight="700" fontFamily="monospace">least stable single channel</text>
            </svg>
            <p className="rcCap">{t("rel.cap", "When one upstream errors, the router recovers on the next-best official channel — you see one flat line.")}</p>
          </div>
        </div>
      </section>
      <section className="why" id="why">
        <PixelGrid cell={18} cols={7} n={9} rows={4} seed={113} style={{ bottom: 30, left: 34 }} />
        <div className="whyIn">
          <div className="whyHead"><h2 className="display" style={{ fontSize: 56 }}>{renderLineBreakText(t("why.h2", "Why choose<br>flatkey?"))}</h2></div>
          <div className="whyGrid">
            <div className="wcard"><h3>{t("why.c1h", "As low as 40% of list")}</h3><p>{t("why.c1p", "National-lab models from 60% of list, stacking with the top-up bonus to 40%; flagships bill at list with the same bonus. One prepaid balance across every model.")}</p><div className="mini"><div className="mbar"><span>official</span><div className="mtrack"><div className="mfill" style={{ width: "100%", background: "#C9C9D2" }} /></div><b>$5.00</b></div><div className="mbar"><span>flatkey</span><div className="mtrack"><div className="mfill" style={{ width: "53%", background: "linear-gradient(90deg,var(--violet-hi),var(--violet-deep))" }} /></div><b style={{ color: "var(--violet-deep)" }}>$2.67</b></div><p className="mcap">claude-opus-4-8 · $/1M input tokens, after bonus</p></div></div>
            <div className="wcard"><h3>{t("why.c2h", "Official endpoints, verified hourly")}</h3><p>{t("why.c2p", "Every request hits the real lab API — no fp8 re-serves, no silent swaps. 300+ models probed against official fingerprints, on a public log.")}</p><div className="labs">{[["G", "OpenAI"], ["C", "Anthropic"], ["G", "Google"], ["D", "DeepSeek"], ["Q", "Alibaba"], ["Z", "Z.ai"]].map(([letter, lab]) => <div className="lab" key={lab} style={{ background: lab === "Anthropic" ? "#B45309" : lab === "Google" ? "#2563EB" : lab === "DeepSeek" ? "#3B52D4" : lab === "Alibaba" ? "var(--violet-deep)" : lab === "Z.ai" ? "#0B0B0F" : "#0E8A6C" }}>{letter}<span>{lab}</span></div>)}</div></div>
            <div className="wcard dark"><h3>{t("why.c3h", "CLI-native. Powered by SKILL.md.")}</h3><p>{t("why.c3p", "Point Codex, Claude Code, OpenClaw, or any agent CLI at Flatkey’s SKILL.md. It learns how to discover and call 300+ official models and 1,000+ pay-per-call tools with one key.")}</p><pre className="wcode"><span>$</span> codex <em>{`"Set up Flatkey from\n  https://flatkey.ai/SKILL.md"`}</em>{"\n\n"}<span>✓</span> 300+ official models connected{"\n"}<span>✓</span> 1,000+ tools discovered{"\n"}<span>✓</span> one key · one balance</pre></div>
            <div className="wcard"><h3>{t("why.c4h", "Enterprise-grade governance")}</h3><p>{t("why.c4p", "Tree-structured sub-keys with budgets and model allowlists, a per-request ledger API, invoices in 48h — on audited infrastructure.")}</p><div className="chips">{["Sub-key caps", "Model allowlists", "Ledger API", "Invoices 48h", "SOC 2 Type II", "ISO 27001", "GDPR", "Zero retention"].map((chip) => <span key={chip}>{chip}</span>)}</div></div>
          </div>
        </div>
      </section>
      <section className="v" id="compliance">
        <PixelGrid accent="#22C55E" cell={24} colors={["#7C3AED66", "#8B5CF64D", "#A78BFA59"]} cols={12} n={20} rows={6} seed={43} style={{ bottom: 70, right: 34, zIndex: 1 }} />
        <div className="bgv ledger">
          <div className="lcol" style={{ left: "56%" }}>req_8f3a2e91 · in 1,204 tok · out 388 tok · cached 61% · $0.0041<br />req_c11977ab · in 640 tok · out 1,022 tok · cached 38% · $0.0009<br />req_40deb0c2 · in 8,441 tok · out 96 tok · cached 95% · $0.0002<br />invoice #2026-0714 · VAT · issued 48h<br />req_77ab40de · 3DS ✓ · stripe checkout · BRL R$55,00<br />audit_export.csv · 128,441 rows · sha256 ✓</div>
          <div className="lcol" style={{ animationDelay: "-8s", left: "75%" }}>subkey cline-agent · cap $50/mo · model allowlist ✓<br />subkey prod · cap $500/mo · alert 80% ✓<br />SLA 99.9% · failover 3-region · breaker 99.95%<br />gdpr export · user 1835 · 2.1MB · done<br />req_b0c240de · in 96 tok · out 2,048 tok · $0.0031</div>
        </div>
        <div className="shade" />
        <div className="in">
          <div className="kick">{t("compl.kick", "COMPLIANCE · ENTERPRISE-GRADE AUDITABILITY")}</div>
          <h2 className="big">{renderInlineEm(t("compl.h2", "Every token,<br>on the record<em>.</em>"))}</h2>
          <p className="d">{t("compl.p", "Per-request ledger with input, output and cached tokens — exportable, API-accessible. Invoices in 48h (VAT fapiao supported), 3DS payments, sub-key caps and model allowlists for teams.")}</p>
          <div className="badges">
            <div className="badge"><b className="num">99.5% SLA</b><span>multi-provider failover · signed</span></div>
            <div className="badge"><b>3DS · Stripe</b><span>fraud-screened payments</span></div>
            <div className="badge"><b>Invoices 48h</b><span>enterprise invoicing API</span></div>
            <div className="badge"><b>Ledger API</b><span>every request, every token</span></div>
            <div className="badge"><b>Sub-key governance</b><span>caps · allowlists · alerts</span></div>
          </div>
        </div>
        <div className="vtag"><span className="p">▶</span>BACKGROUND VIDEO · live ledger stream · 15s loop</div>
      </section>
      <section className="v" id="models">
        <PixelGrid accent="#22C55E" cell={24} colors={["#7C3AED66", "#8B5CF64D", "#A78BFA59"]} cols={10} n={16} rows={5} seed={47} style={{ bottom: 56, right: 60, zIndex: 1 }} />
        <div className="bgv wall">
          {modelWallRows.map((row, rowIndex) => (
            <div className={`lane${rowIndex === 1 ? " r" : ""}`} key={rowIndex}>
              {[...row, ...row].map(([name, meta, price, highlighted, verified], index) => (
                <div className="tile" key={`${name}-${index}`} style={highlighted ? { borderColor: "#8B5CF666" } : undefined}>
                  <b style={highlighted ? { color: "#B7A3F0" } : undefined}>{name}</b>
                  <span>{verified ? <><i style={{ color: "var(--green)", fontStyle: "normal" }}>✓ verified</i> · {meta}</> : meta}</span>
        ...
        <div className="shade" />
        <div className="in">
          <div className="kick">{t("mdl.kick", "MODELS · 300+ OFFICIAL MODELS, ONE KEY")}</div>
          <h2 className="big">{renderInlineEm(t("mdl.h2", "Every frontier lab<em>.</em><br>One key."))}</h2>
          <p className="d">{t("mdl.p", "OpenAI, Anthropic, Google, DeepSeek, Alibaba, Zhipu, Moonshot, ByteDance… new models live within 24h of official release — always the real endpoints, never quantized re-serves.")}</p>
        </div>
        <div className="vtag"><span className="p">▶</span>BACKGROUND VIDEO · model wall marquee · 30s loop</div>
      </section>
      <section className="media" id="video">
        <PixelGrid cell={20} cols={10} n={11} rows={3} seed={103} style={{ right: 36, top: 14 }} />
        <div className="mediaIn">
          <div className="kick2">{t("vid.kick", "VIDEO MODELS · SAME KEY, SAME BALANCE")}</div>
          <h2 className="display">{t("vid.h2", "Video generation, official endpoints.")}</h2>
          <p className="sub" style={{ marginTop: 12, maxWidth: 620 }}>{t("vid.sub", "Text-to-video and image-to-video from every major lab — metered per second on the same prepaid balance as your text models.")}</p>
          <div className="vgal">
            {videoCards.map(([asset, title, subtitle, meta, status, href, featured]) => (
              <Link className={`vcard${featured ? " big" : ""}`} href={localizePath(href, props.locale)} key={asset}>
                <video autoPlay muted loop playsInline preload="auto" poster={`/assets/video/${asset}.jpg`} src={`/assets/video/${asset}.mp4`} />
                <div className="vshade" />
                {featured && <span className="fbadge">{t("vid.badge", "FEATURED · NEW")}</span>}
                <div className="vmeta"><h3>{title}<br />{subtitle}</h3><p><span>{meta}</span><i>{videoStatus(status)}</i></p></div>
              </Link>
            ))}
          </div>
          <div className="mgrid" style={{ marginTop: 16 }}>
            {upcomingVideoModels.map(([model, meta]) => <div className="mcard" key={model}><b>{model}</b><span>{meta}</span><i>{t("md.soon", "Coming soon")}</i></div>)}
            <div className="mcard more"><Link href={localizePath("/models", props.locale)}>{t("vid.more", "+ 20 more video models →")}</Link></div>
          </div>
        </div>
      </section>
      <section className="support" id="support">
        <PixelGrid cell={18} cols={7} n={8} rows={3} seed={97} style={{ bottom: 20, left: 40 }} />
        <div className="supportIn">
          <div><div className="kick2">{t("sup.kick", "SUPPORT · HUMANS ON CALL")}</div><h2 className="display">{t("sup.h2", "Questions? Talk to us.")}</h2><p className="sub" style={{ marginTop: 14, maxWidth: 420 }}>{t("sup.sub", "Integration, model choice, or billing — online support and usage consultation for everything on flatkey.")}</p></div>
          <div className="chGrid">
            <a className="ch" href="https://discord.gg/VrbZFDXj5g" style={{ gridColumn: "1/-1", background: "linear-gradient(120deg,#5865F2 0%,#4752C4 100%)", borderColor: "transparent" }}><b style={{ color: "#fff" }}>{t("sup.dc", "Discord community")}</b><span style={{ color: "#E4E6FF" }}>discord.gg/VrbZFDXj5g</span><i style={{ color: "#C7CCFF" }}>{t("sup.dc.i", "Builders answering builders — the team is in the channels daily")}</i></a>
            <a className="ch" href="mailto:support@flatkey.ai"><b>{t("sup.email", "Email")}</b><span>support@flatkey.ai</span><i>{t("sup.email.i", "Reply within 1 business day")}</i></a>
            <Link className="ch" href={localizePath("/contact", props.locale)}><b>{t("sup.chat", "Live chat")}</b><span>{t("sup.chat.s", "Start chatting →")}</span><i>{t("sup.chat.i", "Fastest answer, on the site")}</i></Link>
            <a className="ch" href="https://x.com/flatkey101"><b>{t("sup.x", "X (Twitter)")}</b><span>@flatkey101</span><i>{t("sup.x.i", "Follow or DM")}</i></a>
          </div>
        </div>
      </section>

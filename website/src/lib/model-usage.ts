 ...
 *
 * Every one of those paths used to return a deterministic invented curve
 * instead (weekday/weekend shaped, seeded from the model name), flagged
 * `placeholder: true` for callers to label. In practice the page charted ~23K
 * requests for a model with none, under the heading "Daily seedance-2.5
 * requests on Flatkey" -- a public SEO surface asserting platform volume that
 * was never measured. A missing section is the honest answer, so there is no
 * stand-in series any more.
 *
 * Note that "no traffic" is a normal, authorised answer, not an error: the feed
 * returns success with an empty series for any model nobody has called yet.
 */
export async function fetchModelUsage(modelName: string, days = 30): Promise<ModelUsage | null> {
  ...
  // Without a key the real feed is unreachable, and there is nothing to chart.
  if (!key) return null;

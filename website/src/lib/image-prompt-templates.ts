...
/**
 * Prompt-card copy is data rather than UI chrome, so it does not pass through
 * the page translation hook. Keep the six workflow briefs localized here and
 * let the page request the locale-aware catalog explicitly. Bracketed values
 * remain unchanged because they are intentional input slots for the user.
 */
...
export function getImagePromptTemplates(_modelId?: string, locale: Locale = "en"): ImagePromptTemplate[] {
  ...
  return IMAGE_PROMPT_TEMPLATES.map((template) => ({
    ...
    prompt: copy?.prompts[template.id] ?? template.prompt,
    tags: [...template.tags],
  }));
}
...
export function getImagePlaygroundExample(modelId = "", locale: Locale = "en"): ImagePlaygroundExample | undefined {
  ...
  if (resolvedId && IMAGE_PLAYGROUND_EXAMPLES[resolvedId]) {
    ...
    const subject = IMAGE_PLAYGROUND_SCENARIOS[resolvedId]?.[locale] ?? IMAGE_PLAYGROUND_SCENARIOS[resolvedId]?.en;
    const modelLabel = modelId.trim() || resolvedId;
    return {
      ...example,
      prompt: subject ? IMAGE_PLAYGROUND_PROMPT_FRAME[locale](modelLabel, subject) : example.prompt,
    };
  }
...
export function localizeImagePromptText(prompt: string, locale: Locale): string {
  ...
  if (generic && locale !== "en") {
    const localizedFrame = IMAGE_PLAYGROUND_PROMPT_FRAME[locale];
    const modelName = generic[1].trim();
    const subject = generic[2].trim();
    return localizedFrame(modelName, subject);
  }
  return prompt;
}

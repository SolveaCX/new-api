...
func TestSetChannelSupplierContractCASPreventsStaleWritesAndKeepsRetriesIdempotent(t *testing.T) {
	...
	require.ErrorIs(t, SetChannelSupplierContractCAS(channel.Id, 0, &first.Id), ErrSupplierBindingChanged, "stale expected state cannot succeed as an untracked desired-state no-op")
	...
	require.ErrorIs(t, SetChannelSupplierContractCAS(channel.Id, first.Id, &second.Id), ErrSupplierBindingChanged, "only command-ledger replay may bypass a stale observed state")
	...
	require.ErrorIs(t, SetChannelSupplierContractCAS(channel.Id, second.Id, nil), ErrSupplierBindingChanged, "stale unbind no-op is not an idempotent replay")
...
func TestSupplierAdminProjectionsAreEnrichedFilteredAndBounded(t *testing.T) {
	...
	require.Equal(t, int64(1), suppliers[0].RowVersion)
	...
	require.Equal(t, int64(2), contracts[0].RowVersion, "rate activation advances the contract CAS version exposed by the list projection")

...
type PerfMetric struct {
	...
	AvailabilityEligibleCount int64  `json:"-" gorm:"default:0"`
	AvailabilitySuccessCount  int64  `json:"-" gorm:"default:0"`
}
...
func UpsertPerfMetric(metric *PerfMetric) error {
	...
	return DB.Clauses(clause.OnConflict{
		...
		DoUpdates: clause.Assignments(map[string]interface{}{
			...
			"availability_eligible_count": gorm.Expr("perf_metrics.availability_eligible_count + ?", metric.AvailabilityEligibleCount),
			"availability_success_count":  gorm.Expr("perf_metrics.availability_success_count + ?", metric.AvailabilitySuccessCount),
		}),
	}).Create(metric).Error
}
...
type PerfMetricSummary struct {
	...
	AvailabilityEligibleCount int64  `json:"availability_eligible_count"`
	AvailabilitySuccessCount  int64  `json:"availability_success_count"`
}
...
func GetPerfMetricsSummaryAll(startTs int64, endTs int64, groups []string) ([]PerfMetricSummary, error) {
	...
	query := DB.Model(&PerfMetric{}).
		Select("model_name, SUM(request_count) as request_count, SUM(success_count) as success_count, SUM(total_latency_ms) as total_latency_ms, SUM(ttft_sum_ms) as ttft_sum_ms, SUM(ttft_count) as ttft_count, SUM(output_tokens) as output_tokens, SUM(generation_ms) as generation_ms, SUM(availability_eligible_count) as availability_eligible_count, SUM(availability_success_count) as availability_success_count").

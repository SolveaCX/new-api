...
func TestStripeSubscriptionWebhookUpdatedRoutesTerminalSnapshotToTermination(t *testing.T) {
	testCases := []struct {
		name    string
		status  string
		endedAt int64
	...
	for index, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			binding := insertStripeFulfillmentSubscriptionBinding(t, userID, providerSubscriptionID, "active", false)
			_, _, err := model.ReserveSubscriptionProviderLifecycle(
				...
				"webhook-terminal-reservation-"+testCase.name,
				300,
			)
			require.NoError(t, err)
			originalSnapshot := stripeSubscriptionSnapshotFromSubscriptionEvent
